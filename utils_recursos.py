...
#       BAG of WORDS
# ------------------------------
def obtener_vector_bag_fit_transform(data):
    english_stopwords = stopwords.words('english')
    vectorizer = CountVectorizer(
                analyzer = 'word',
                stop_words = english_stopwords)
    result = vectorizer.fit_transform(data).toarray()
    return result

def obtener_vector_bag_test(data):
    english_stopwords = stopwords.words('english')
    vectorizer = CountVectorizer(
                analyzer = 'word',
                stop_words = english_stopwords)
    bag = vectorizer.fit_transform(data).toarray()
    result = obtener_categorias_diccionario(bag)
    return result
